...
@section('admin-content')
    <div class="max-w-4xl mx-auto px-4 py-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-blue-400">Nuevo video</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <!-- Form -->
        <form action="{{ route('videos.manage.store') }}" method="POST" class="bg-slate-800 rounded-xl shadow-lg p-6">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" required
                          class="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-white"
                          rows="4" placeholder="Descripción del video">{{ old('description') }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
