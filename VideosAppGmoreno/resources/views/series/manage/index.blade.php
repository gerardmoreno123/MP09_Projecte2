    ...
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
            <h1 class="text-2xl font-bold text-white">
                <i class="fas fa-film text-blue-400 mr-2" aria-hidden="true"></i>
                Gestión de Series
            </h1>

            <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 w-full md:w-auto">
                <a href="{{ route('series.manage.index') }}"
                   class="btn {{ request()->routeIs('series.manage.index') ? 'btn-primary' : 'btn-secondary' }}">
                    Listado
                </a>
                <a href="{{ route('series.manage.create') }}"
                   class="btn {{ request()->routeIs('series.manage.create') ? 'btn-primary' : 'btn-secondary' }}">
                    Nueva Serie
                </a>
            </div>
        </div>
        ...
        <div class="mb-6 bg-slate-800 p-4 rounded-xl shadow-md flex flex-col md:flex-row gap-4 items-center">
            <form action="{{ route('series.manage.index') }}" method="GET" class="flex-1 flex gap-4">
            ...
            @if(request('search'))
                <a href="{{ route('series.manage.index') }}" class="btn btn-ghost" aria-label="Limpiar búsqueda">
            ...
            <div class="hidden sm:block">
                <table class="w-full text-left">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 font-medium text-white text-center">Videos</th>
                        <th scope="col" class="px-6 py-4 font-medium text-white">Creada</th>
                        <th scope="col" class="px-6 py-4 font-medium text-white text-right">Acciones</th>
                    </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-700">
                    @forelse($series as $serie)
                        <tr onclick="window.location='{{ route('series.manage.show', $serie) }}';" class="hover:bg-slate-700/50 transition-colors cursor-pointer">
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @if($serie->description)
                                            <div class="text-sm text-slate-400 whitespace-pre-line">{{ $serie->description }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                                <div class="bg-slate-800 rounded-xl">
                                    ...
                                    <h3 class="text-xl font-medium text-white mb-2">No hay series creadas</h3>
                                    <p class="text-slate-400">Aún no se han creado series en la plataforma.</p>
                                    <a href="{{ route('series.manage.create') }}"
                                       class="inline-block mt-4 btn btn-primary">
                                        <i class="fas fa-plus mr-2" aria-hidden="true"></i> Crear nueva serie
                                    </a>
                                </div>
                            </td>
                        </tr>
            ...
            <div class="block sm:hidden space-y-4 p-4">
                @forelse($series as $serie)
                    <div class="bg-slate-900 rounded-lg p-4 shadow-md hover:bg-slate-700/50 transition-colors cursor-pointer"
                         onclick="window.location='{{ route('series.manage.show', $serie) }}';">
                        ...
                        <div class="space-y-2">
                            @if($serie->description)
                                <div class="flex justify-between items-start">
                                    <span class="text-sm text-slate-400">Descripción:</span>
                                    <span class="text-sm text-white whitespace-pre-line text-right">{{ $serie->description }}</span>
                                </div>
                            @endif
                            ...
                            <div class="flex justify-between items-center">
                                <span class="text-sm text-slate-400">Creada:</span>
                                <div class="text-right text-sm text-slate-400">
                                    <div>{{ $serie->created_at->format('d/m/Y') }}</div>
                                    <div class="text-xs">{{ $serie->created_at->diffForHumans() }}</div>
                                </div>
                            </div>
